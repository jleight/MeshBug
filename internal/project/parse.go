...
type packetMessage struct {
	...
	// some observers publish this as a JSON number (e.g. 0), others as a string
	PacketType flexStr `json:"packet_type"`
	...
	// observers may include the accumulated path as a hex string, e.g. "F0 B7"
...
func parseTopicHash(
	prefix, topic string,
) (id []byte, region string, kind string, err error) {
...
func atoiFlex(f flexStr) *int {
	return atoiPtr(string(f))
}

func atofFlex(f flexStr) *float64 {
	return atofPtr(string(f))
}
