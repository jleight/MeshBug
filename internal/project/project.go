...
// processed. On startup we resume from there; on each loop iteration we
// commit advancing cursor + derived inserts in the same transaction conceptually
// (advance cursor only after successful writes for the batch).
package project
...
func (p *Projector) Run(ctx context.Context, databaseURL string) error {
	...
	// if it falls behind or disconnects, the catch-up loop on next wake closes
	// the gap.
	errs := make(chan error, 1)
	go func() {
		errs <- notify.Listen(ctx, databaseURL, []string{ingest.NotifyChannel}, p.log,
			func(_ notify.Notification) {
				if err := p.catchUp(ctx, &cursor); err != nil && !errors.Is(err, context.Canceled) {
	...
	for {
		...
		if len(packets) > 0 {
			if _, err := p.store.InsertPacketBatch(ctx, packets); err != nil {
				p.log.Error("packet batch write failed", "n", len(packets), "err", err)
			...
			_ = notify.Publish(ctx, p.store.Pool, notify.ChannelPackets, map[string]any{
				"count":     len(packets),
				"observers": ids,
			})
		}
		for k := range statusAffected {
			_ = notify.Publish(ctx, p.store.Pool, notify.ChannelStatus, map[string]any{
...
func (p *Projector) applyStatus(ctx context.Context, e store.RawEvent, obsID []byte) error {
	m, err := unmarshalStatus(e.Payload)
	if err != nil {
		p.log.Warn("bad status json", "id", e.ID, "err", err)
		return err
	}
	region := topicRegion(e.Topic)
	freq, bw, sf, cr := parseRadio(m.Radio)
	up := store.ObserverUpsert{
		ID: obsID, OriginName: m.Origin, Region: region, Model: m.Model,
		FirmwareVersion: m.FirmwareVersion, ClientVersion: m.ClientVersion, Source: m.Source,
	...
	row := store.StatusRow{
		ObserverID: obsID, TS: e.ReceivedAt, Status: m.Status,
...
func (p *Projector) decodePacket(e store.RawEvent, obsID []byte) (store.PacketRow, bool) {
	m, err := unmarshalPacket(e.Payload)
	if err != nil {
		p.log.Warn("bad packet json", "id", e.ID, "err", err)
	...
	if len(rawBytes) > 0 {
		...
		default:
			p.log.Debug("decode failed", "id", e.ID, "err", err)
		}
	}
	return row, true
}

// Keep go vet happy if any of these are temporarily unused while we iterate.
var _ = time.Second
