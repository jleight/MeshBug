...
func (w *Worker) rollup1mRange(ctx context.Context, from, to time.Time) error {
	_, err := w.store.Pool.Exec(
		ctx,
		`
		WITH agg AS (
		    SELECT   p.observer_id
		...
		INSERT INTO rollup_observer_1m
		    (observer_id
		    ,bucket
		    ,packets
		    ,unique_pkts
		...
		SELECT   a.observer_id
		        ...
		        ,a.max_rssi
		        ,a.avg_snr
		        ,(SELECT s.noise_floor
		          FROM   observer_status s
		          WHERE  s.observer_id = a.observer_id
		             AND s.ts >= a.bucket
		             AND s.ts <  a.bucket + interval '1 minute'
		          ORDER BY s.ts DESC
		          LIMIT 1)
		FROM    agg a
...
func (w *Worker) rollup1hRange(ctx context.Context, from, to time.Time) error {
	_, err := w.store.Pool.Exec(
		ctx,
		`
		INSERT INTO rollup_observer_1h
		    (observer_id
		    ,bucket
		    ,packets
		    ,unique_pkts
