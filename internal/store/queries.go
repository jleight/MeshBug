...
func (s *Store) InsertPacketBatch(ctx context.Context, rows []PacketRow) (int64, error) {
	...
	// Maintain packets_unique. Dedupe by hash *within* this batch first.
	// Postgres can't update the same conflict target twice in one statement.
	// We want observer_count to bump by exactly one per upsert call no
	// matter how many observers in this batch happened to hear the same
	// packet. The rollup + per-observer table give the per-observer counts
	// anyway.
	type uagg struct {
		first time.Time
		ptype string
