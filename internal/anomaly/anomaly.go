...
func (w *Worker) observerSilent(ctx context.Context, now time.Time) {
	rows, err := w.store.Pool.Query(
		...
		WITH baseline AS (
		    SELECT   observer_id
		            ,AVG(packets) AS avg_per_min
		    FROM    rollup_observer_1m
		    WHERE   bucket >= $1::timestamptz - interval '24 hours'
		        AND bucket <  $1::timestamptz - interval '15 minutes'
		    GROUP BY observer_id
...
func (w *Worker) rssiDrop(ctx context.Context, now time.Time) {
	rows, err := w.store.Pool.Query(
		...
		SELECT   b.observer_id
		        ...
		        ,r.rssi_1h
		FROM    base b
		JOIN    recent r USING (observer_id, source_prefix)
...
func (w *Worker) trafficSpike(ctx context.Context, now time.Time) {
	rows, err := w.store.Pool.Query(
		...
		WITH base AS (
		    SELECT   packet_type
		            ,COUNT(*)::float
		             / GREATEST(EXTRACT(EPOCH FROM (interval '23 hours')) / 3600, 1) AS per_hr
		    FROM    packet_observations
		    WHERE   ts >= $1::timestamptz - interval '24 hours'
		        AND ts <  $1::timestamptz - interval '1 hour'
		...
		SELECT   b.packet_type
		        ,b.per_hr
		        ,r.cnt
		FROM    base b
		JOIN    recent r USING (packet_type)
