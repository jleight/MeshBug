...
func runWeb(_ []string) {
	...
	// from project land here; we re-publish to the relevant in-process topics
	...
	go func() {
		...
		handle := func(n notify.Notification) {
			bridgeNotification(n, hub)
		}

		err := notify.Listen(ctx, cfg.DatabaseURL, channels, log, handle)
